...
class freeswitch_server(orm.Model):
    ...
    _columns = {
        ...
        'port': fields.integer(
            'Port', required=True,
            help="TCP port on which the FreeSWITCH Manager Interface listens. "
            "Defined in /usr/local/freeswitch/conf/autoload_configs/event_socket.conf.xml on FreeSWITCH."),
        ...
        'password': fields.char(
            ...
            "FreeSWITCH Manager Interface. Refer to /usr/local/freeswitch/conf/autoload_configs/event_socket.conf.xml "
        ...
        'context': fields.char(
            ...
            "made; e.g. 'XML default'. Refer to /usr/local/freeswitch/conf/dialplan/* on your FreeSWITCH "
            "server."),
    ...
    def _connect_to_freeswitch(self, cr, uid, context=None):
        ...
        try:
            ...
            _logger.error(
                "Error in the request to the FreeSWITCH Manager Interface %s"
    ...
    def test_es_connection(self, cr, uid, ids, context=None):
        ...
        raise orm.except_orm(
            ...
            _("OpenERP can successfully login to the FreeSWITCH Manager "
                "Interface."))
    ...
    def _get_calling_number(self, cr, uid, context=None):
        ...
        try:
            ...
            for row in reader:
                if not row.has_key("uuid") or row["uuid"] == "" or row["uuid"] == "uuid":
                    break
...
#                variable = variable)
            dial_string = (('<' + variable + '>') if variable else '') + channel + ' ' + fs_number + ' ' + fs_server.context + ' ' + fs_number + ' ' + fs_number
