...
"""
 ...
 This script is designed to be used as an AGI on an FreeSWITCH IPBX...
 ...
 available from https://code.launchpad.net/openerp-freeswitch-connector
 ...
 For that, it uses the CONNECTEDLINE dialplan function of FreeSWITCH
 See the following page for more info:
 https://wiki.freeswitch.org/wiki/display/AST/Manipulating+Party+ID+Information

 It is not possible to set the CONNECTEDLINE directly from an AGI script,
 (at least not with FreeSWITCH 11) so the AGI script sets a variable
 ...
 [openerp-out-call]
 exten = s,1,AGI(/var/lib/freeswitch/agi-bin/set_name_outgoing_timeout.sh)
...
options = [
    ...
    {'names': ('-o', '--outgoing'), 'dest': 'outgoing', 'help': "Update the Connected Line ID name on outgoing calls via a call to the FreeSWITCH function CONNECTEDLINE(), instead of updating the Caller ID name on incoming calls. Default = no.", 'action': 'store_true', 'default': False},
...
def stdout_write(string):
    ...
    # Purge stdin to avoid these FreeSWITCH error messages :
    # utils.c fs_carefulwrite: write() returned error: Broken pipe
    sys.stdin.readline()
    return True
