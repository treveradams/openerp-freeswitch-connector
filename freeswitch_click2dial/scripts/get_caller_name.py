...
"""
 ...
 If you want geoloc add &geoloc=true to the end (it is going to be slow). Notify should be the internal Number of the called parties. This should be comma (,) delimited, not :_: delimted. It is up to you to format the extensions list appropriately.

 From the dialplan, do something like this <action application="set" data="effective_caller_id_name=${cidlookup(${caller_id_number})}"/>.
 If you are not using FreeTDM modules for the incoming line, doing <action application="pre_answer"/> before the cidlookup is a VERY good idea

 If you are wishing to set the callee name, <action application="export" data="callee_id_name=${cidlookup($1)}" />
